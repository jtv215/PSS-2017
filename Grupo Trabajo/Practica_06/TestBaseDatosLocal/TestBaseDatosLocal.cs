...
{
    [TestClass]
    public class TestBaseDatosLocal
    {

        //Test para comprobar conexion
        [TestMethod]
        public void SqlServerFile_ProbarConexion()
        {
            string connectionString = @"data source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\AutentificacionDB.mdf;Integrated Security=True";
            try
            {
        ...
        //Test para comprobar la correcta inserción de un registro en la base de datos local.       
        ...
        public void SqlServerFile_SqlInsertar()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    if (result != 1)
                        throw new Exception("Error al insertar un nuevo registro. ");
                }
            }
            catch (Exception ex)
            {
        ...
        //Test para comprobar la correcta eliminar Insertado.
        [TestMethod()]
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    if (!Convert.IsDBNull(objId))
                        MaxId = Int32.Parse(objId.ToString());
                    ...
                    if (result != 1)
                        throw new Exception("Error al Eliminar un nuevo registro. ");
                }
            }
            catch (Exception ex)
            {
        ...
        //Test para comprobar modificar Insertado.
        [TestMethod()]
        public void SqlServerFile_SqlModificarInsertado()
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    cmdMod.CommandText = "Update Autentificaciones set Nombre = @newNombre ,PalabraPaso = @newPalabraPaso, "
                        + " Categoria = @newCategoria , EsValido = @newEsValido WHERE Id = @IdUsuario ";
                    ...
                    if (result != 1)
                        throw new Exception("Error al Modificar un registro. ");
                }
            }
            catch (Exception ex)
            {
